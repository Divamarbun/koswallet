...
function TxModal({ onClose, onSave }) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    await onSave({ type:form.type, desc:form.desc, amount:parseFloat(form.amount), cat:form.cat, date:new Date().toISOString().split("T")[0] });
    setSaving(false);
    onClose();
  };
  return (
    <div style={S.overlay} onClick={onClose}>
      <div style={S.modal} onClick={e=>e.stopPropagation()}>
        ...
        <div style={S.btnRow}>
          ...
          <button style={S.btnSave} onClick={save} disabled={saving}>{saving?"Menyimpan...":"Simpan"}</button>
        </div>
      </div>
    </div>
  );
...
function SavingModal({ onClose, onSave }) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    await onSave({ name:form.name, target:parseFloat(form.target), current:parseFloat(form.current)||0, emoji:form.emoji });
    setSaving(false);
    onClose();
  };
  return (
...
function HomeView({ txs, loading, onAdd, onDelete }) {
  ...
  return (
    <>
      ...
      {loading
        ? <div style={S.loadingWrap}><Loader2 size={18} style={{animation:"spin 1s linear infinite"}}/> Memuat data...</div>
        : <div style={S.txList}>
            {recent.length===0
              ? <div style={{color:"#8884a8",fontSize:13,textAlign:"center",padding:"20px 0"}}>Belum ada transaksi. Tambah sekarang!</div>
              : recent.map(t=>(
      ...
      <button style={S.addBtn} onClick={onAdd}><Plus size={16}/> Tambah Transaksi</button>
    </>
  );
}
...
function SavingView({ savings, loading, onAdd }) {
  ...
  return (
    <>
      ...
      {loading
        ? <div style={S.loadingWrap}><Loader2 size={18} style={{animation:"spin 1s linear infinite"}}/> Memuat data...</div>
        : savings.length===0
          ? <div style={{color:"#8884a8",fontSize:13,textAlign:"center",padding:"20px 0"}}>Belum ada target tabungan.</div>
          : savings.map((s,i)=>{
      ...
      <button style={S.addBtn} onClick={onAdd}><Plus size={16}/> Tambah Target</button>
    </>
  );
}
...
function AiView({ txs, savings }) {
  ...
  const getTip = async () => {
    ...
    } catch { setTip("⚠️ Gagal memuat tips. Coba lagi."); }
    setLoading(false);
  };
  ...
  return (
    <>
      ...
      <div style={{...S.card,border:"1px solid rgba(79,209,165,0.2)"}}>
        ...
        {[["🍱","Masak sendiri bisa hemat hingga 60% pengeluaran makan"],["🚶","Jalan kaki atau naik sepeda untuk jarak dekat"],["📱","Manfaatkan promo & cashback aplikasi dompet digital"],["☕","Kurangi jajan kopi, buat sendiri di kos"]].map(([e,t])=>(
...
export default function App() {
  ...
  useEffect(() => { fetchTxs(); fetchSavings(); }, []);

  const fetchTxs = async () => {
    setLoadingTx(true);
    const { data } = await supabase.from("transactions").select("*").order("created_at",{ascending:false});
    setTxs(data||[]); setLoadingTx(false);
  };

  const fetchSavings = async () => {
    setLoadingSv(true);
    const { data } = await supabase.from("savings").select("*").order("created_at",{ascending:true});
    setSavings(data||[]); setLoadingSv(false);
  };
